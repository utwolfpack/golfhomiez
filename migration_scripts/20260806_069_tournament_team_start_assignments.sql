-- Stores editable team start assignments for GolfHomiez tournaments.
-- tournament_id is created with the exact type/charset/collation of tournaments.id
-- so the foreign key remains compatible in databases that use utf8mb4_bin IDs.
-- The application migration runner also performs idempotent reconciliation for
-- partially migrated databases.

SET @tournament_id_definition := (
  SELECT CONCAT(
    UPPER(COLUMN_TYPE),
    IF(CHARACTER_SET_NAME IS NULL, '', CONCAT(' CHARACTER SET `', REPLACE(CHARACTER_SET_NAME, '`', '``'), '`')),
    IF(COLLATION_NAME IS NULL, '', CONCAT(' COLLATE `', REPLACE(COLLATION_NAME, '`', '``'), '`')),
    ' NOT NULL'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'tournaments'
    AND COLUMN_NAME = 'id'
  LIMIT 1
);

SET @sql := IF(
  @tournament_id_definition IS NULL OR TRIM(@tournament_id_definition) = '',
  'SIGNAL SQLSTATE ''45000'' SET MESSAGE_TEXT = ''Cannot create tournament start assignments because tournaments.id metadata is unavailable''',
  CONCAT(
    'CREATE TABLE IF NOT EXISTS tournament_team_start_assignments (',
    'id VARCHAR(191) NOT NULL PRIMARY KEY,',
    'tournament_id ', @tournament_id_definition, ',',
    'team_key VARCHAR(255) NOT NULL,',
    'registration_id VARCHAR(191) NULL,',
    'team_id VARCHAR(191) NULL,',
    'team_name VARCHAR(191) NOT NULL,',
    'start_type VARCHAR(32) NOT NULL DEFAULT ''shotgun'',',
    'start_time TIME NOT NULL,',
    'starting_hole VARCHAR(12) NULL,',
    'sort_order INT UNSIGNED NOT NULL DEFAULT 0,',
    'notes VARCHAR(500) NULL,',
    'updated_by_auth_user_id VARCHAR(191) NULL,',
    'correlation_id VARCHAR(191) NULL,',
    'created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,',
    'updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,',
    'UNIQUE KEY uq_tournament_team_start_assignment (tournament_id, team_key),',
    'KEY idx_tournament_team_start_schedule (tournament_id, sort_order, start_time),',
    'KEY idx_tournament_team_start_registration (registration_id),',
    'KEY idx_tournament_team_start_correlation (correlation_id),',
    'CONSTRAINT fk_tournament_team_start_tournament FOREIGN KEY (tournament_id) REFERENCES tournaments(id) ON DELETE CASCADE',
    ') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4'
  )
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

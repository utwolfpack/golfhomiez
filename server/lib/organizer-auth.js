...
export async function createOrganizerPasswordReset(source, { email, resetUrlBase } = {}) {
  const db = getDb(source)
  await ensureOrganizerAuthSchema(db)
  const normalizedEmail = normalizeEmail(email)
  const organizer = await getOrganizerAccountByEmailDirect(db, normalizedEmail)
  if (!organizer?.password_hash) return { ok: true }
  ...
  await db.execute(
    `INSERT INTO organizer_password_reset_tokens (id, organizer_account_id, token_hash, expires_at) VALUES (?, ?, ?, ?)`,
    [id, organizer.id, tokenHash, expiresAt],
  )

  const appOrigin = String(resetUrlBase || process.env.APP_ORIGIN || process.env.CLIENT_ORIGIN || process.env.BETTER_AUTH_URL || '').trim()
  const resetBase = appOrigin.replace(/\/$/, '')
  const resetPath = `/organizer/reset-password?token=${encodeURIComponent(token)}`
  const resetUrl = resetBase ? `${resetBase}${resetPath}` : resetPath
  await sendMail({
    to: organizer.reset_email || organizer.email || normalizedEmail,
...
export async function resetOrganizerPassword(source, { token, password } = {}) {
  ...
  const [rows] = await db.execute(
    `SELECT * FROM organizer_password_reset_tokens
      WHERE token_hash = ?
        AND used_at IS NULL
        AND expires_at > NOW()
      LIMIT 1`,
  ...
  await db.execute('UPDATE organizer_role_accounts SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hashPassword(password), reset.organizer_account_id])
  await db.execute('UPDATE organizer_password_reset_tokens SET used_at = NOW(), updated_at = CURRENT_TIMESTAMP WHERE id = ?', [reset.id])
  return { ok: true }
}

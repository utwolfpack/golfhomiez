...
// U.S. territories and non-geographic/toll-free NPAs are intentionally excluded because
// this helper is used to decide whether a golf-course phone line is a U.S. local number.
...
export function normalizeUsPhoneForDisplay(value) {
  const digits = phoneDigits(value)
  if (!digits) return null
  const areaCode = digits.slice(0, 3)
  const exchange = digits.slice(3, 6)
  if (!US_GEOGRAPHIC_AREA_CODES.has(areaCode)) return null
  if (!/^[2-9]\d{2}$/.test(exchange)) return null
  return `${areaCode} ${exchange} ${digits.slice(6)}`
}

export function isDisplayableUsPhoneNumber(value) {
  return Boolean(normalizeUsPhoneForDisplay(value))
}

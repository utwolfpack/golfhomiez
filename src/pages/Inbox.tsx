...
import { buildClientDefaultHoleScorecard, holeScoreTotal, missingHoleScoreNumbers, normalizeHoleScorecard } from '../lib/hole-scorecard'
...
function challengeStatusLabel(value?: string | null) {
...
export default function Inbox() {
  ...
  const [messageType, setMessageType] = useState<InboxMessageType>('message')
  ...
  function userCanUpdateTeamChallenge(message: InboxMessage) {
    if (message.messageType !== 'challenge_request') return false
    return myTeams.some((team) => team.id === message.proposerTeamId || team.id === message.challengedTeamId)
  }
  ...
  async function handleMessageSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      logFrontendEvent({ category: 'inbox.message', message: 'inbox_send_succeeded', data: { recipientEmail: trimmedRecipient, messageType: 'message', messageId: result.message?.id, threadId: result.message?.threadId } })
      await loadInbox()
    } catch (err) {
      ...
      setError(message)
      logFrontendEvent({ category: 'inbox.message', level: 'error', message: 'inbox_send_failed', data: { recipientEmail: trimmedRecipient, messageType: 'message', error: message } })
    } finally {
      setSending(false)
    }
  }
  ...
  async function handleChallengeSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessageType(messageTypeForChallenge)
  ...
  async function handleAcceptTeamChallenge(message: InboxMessage) {
    setUpdatingChallengeId(message.id)
    setError(null)
    setStatus(null)
    try {
      logFrontendEvent({ category: 'inbox.teamChallenge', message: 'team_challenge_accept_started', data: { messageId: message.id, threadId: message.threadId || message.id, proposerTeamId: message.proposerTeamId, challengedTeamId: message.challengedTeamId } })
      const updated = await updateTeamChallengeStatus(message.id, 'accepted')
      setMessages((prev) => prev.map((item) => (messageThreadId(item) === messageThreadId(updated) ? { ...item, challengeStatus: updated.challengeStatus } : item)))
      setSentChallenges((prev) => prev.map((item) => (messageThreadId(item) === messageThreadId(updated) ? { ...item, challengeStatus: updated.challengeStatus } : item)))
      setStatus('Team Challenge accepted. Team members can continue the dialogue in this thread.')
      logFrontendEvent({ category: 'inbox.teamChallenge', message: 'team_challenge_accept_succeeded', data: { messageId: message.id, threadId: updated.threadId || updated.id, challengeStatus: updated.challengeStatus } })
      await loadInbox()
    } catch (err) {
      const messageText = err instanceof Error ? err.message : 'Could not update Team Challenge.'
      setError(messageText)
      logFrontendEvent({ category: 'inbox.teamChallenge', level: 'error', message: 'team_challenge_accept_failed', data: { messageId: message.id, error: messageText } })
    } finally {
  ...
  async function handleIndividualChallengeScoreSave(message: InboxMessage, participant: IndividualChallengeParticipant) {
    ...
    const missingHoleNumbers = missingHoleScoreNumbers(holes)
    if (missingHoleNumbers.length) {
      const missingText = `Finish entering scores for ${participantDisplayName(participant)} holes: ${missingHoleNumbers.join(', ')}.`
      setError(missingText)
      logFrontendEvent({ category: 'inbox.individualChallenge.scorecard', level: 'warn', message: 'individual_challenge_scorecard_incomplete', data: { messageId: message.id, threadId: message.threadId || message.id, participantEmail: participantEmail(participant), missingHoleNumbers } })
      return
    }

    const score = holeScoreTotal(holes)
    if (!Number.isFinite(score) || score < 0) {
      setError('Individual Challenge score must be zero or greater.')
      return
    }
    ...
    setStatus(null)
    try {
      logFrontendEvent({ category: 'inbox.individualChallenge.score', message: 'individual_challenge_score_update_started', data: { messageId: message.id, threadId: message.threadId || message.id, participantEmail: participantEmail(participant), score, holeCount: holes.length } })
      ...
      setStatus('Individual Challenge score saved.')
      logFrontendEvent({ category: 'inbox.individualChallenge.score', message: 'individual_challenge_score_update_succeeded', data: { messageId: updated.id, threadId: updated.threadId || updated.id, participantEmail: participantEmail(participant), score, holeCount: holes.length } })
      await loadInbox()
    } catch (err) {
      const messageText = err instanceof Error ? err.message : 'Could not save Individual Challenge score.'
      setError(messageText)
      logFrontendEvent({ category: 'inbox.individualChallenge.score', level: 'error', message: 'individual_challenge_score_update_failed', data: { messageId: message.id, participantEmail: participantEmail(participant), score, error: messageText } })
    } finally {
      setUpdatingChallengeScoreKey(null)
    }
  }
  ...
  function renderTeamChallengeContext(message: InboxMessage) {
    ...
    return (
      <div className="inboxTeamChallengeContext">
        ...
        <span className="pill">{challengeStatusLabel(message.challengeStatus)}</span>
  ...
  function renderReadonlyTeamChallengeHoles(holes: HoleScoreDetail[]) {
    const providedHoles = holes.filter((hole) => hole.scoreProvided)
    if (!providedHoles.length) return <div className="small">No hole-by-hole score has been entered yet.</div>
    return (
      <div className="inboxTeamChallengeReadonlyScorecard" aria-label="Read-only Team Challenge hole scores">
        {providedHoles.map((hole) => (
          <div key={hole.hole} className="inboxTeamChallengeReadonlyHole">
          ...
          {editable ? (
            <>
              ...
              <div className="pageHeroActions inboxMessageActions inboxTeamChallengeScorecardActions">
                ...
                <button type="button" className="btn btnSmall" onClick={() => setActiveIndividualChallengeScorecard(null)}>Cancel</button>
              </div>
            </>
          ) : (
            <div className="card holeInputPanel inboxTeamChallengeReadonlyPanel">
  ...
  function renderThreadCard(thread: InboxThread, source: 'messages' | 'team-challenges') {
    ...
    const isIndividualChallengeMessage = message.messageType === 'individual_challenge'
    const latestMessage = getLatestConversationMessage(message)
    const canAcceptTeamChallenge = source === 'team-challenges' && isTeamChallengeMessage && userCanUpdateTeamChallenge(message) && String(message.challengeStatus || 'proposed').toLowerCase() === 'proposed'
    ...
    return (
      <article key={thread.threadId} className={`inboxMessageCard ${thread.unreadCount > 0 ? 'inboxMessageCard--unread' : 'inboxMessageCard--read'} ${isExpanded ? 'inboxMessageCard--expanded' : 'inboxMessageCard--collapsed'}`}>
        ...
        <div className="pageHeroActions inboxMessageActions">
          ...
          {isExpanded && canAcceptTeamChallenge ? <button type="button" className="btn btnSmall" disabled={updatingChallengeId === message.id} onClick={() => void handleAcceptTeamChallenge(message)}>{updatingChallengeId === message.id ? 'Accepting…' : 'Accept Team Challenge'}</button> : null}
        </div>
  ...
  return (
    <div className="container pageStack inboxPage">
      <PageHero
        eyebrow="Golf user inbox"
        title="Inbox"
      />

      <section className="card inboxComposeCard inboxComposeCard--collapsed">
        <button
          type="button"
          className="inboxSectionToggleButton"
          aria-expanded={challengesComposeOpen}
          onClick={() => {
            setChallengesComposeOpen((open) => !open)
            logFrontendEvent({ category: 'inbox.challengeCompose', message: challengesComposeOpen ? 'challenges_form_collapsed' : 'challenges_form_opened' })
          }}
        >
          <span>
            <h2 className="inboxSectionTitle">Challenges</h2>
            <span className="small">Open this form to create Team Challenges or Individual Challenges.</span>
          </span>
          <span className="pill">{challengesComposeOpen ? 'Close form' : 'Open form'}</span>
        </button>

        {challengesComposeOpen ? (
          <form className="formStack inboxCollapsibleForm" onSubmit={handleChallengeSubmit}>
      ...
      <section className="card inboxComposeCard inboxComposeCard--collapsed">
        <button
          ...
          onClick={() => {
            setMessageComposeOpen((open) => !open)
            logFrontendEvent({ category: 'inbox.messageCompose', message: messageComposeOpen ? 'send_message_form_collapsed' : 'send_message_form_opened' })
          }}
        >
          <span>
            <h2 className="inboxSectionTitle">Send a Message</h2>
            <span className="small">Messages can only be sent to existing Golf Homiez users.</span>
          </span>
          <span className="pill">{messageComposeOpen ? 'Close form' : 'Open form'}</span>
        </button>

        {messageComposeOpen ? (
          <form className="formStack inboxCollapsibleForm" onSubmit={handleMessageSubmit}>
            <div>
      ...
      <section className="card inboxListCard">
      ...
      <section className="card inboxListCard">
        <div className="inboxSectionHeader">
          ...
          <span className="pill">{teamChallengeThreads.length} active</span>
        </div>

...
import { requestHostPasswordReset, type PasswordResetDeliveryMethod } from '../lib/host-auth'
...
export default function HostForgotPassword() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setMessage(null)
    try {
      const result = await requestHostPasswordReset(email.trim(), deliveryMethod)
      if (!result.response.ok) throw new Error((result.data as any)?.message || 'Could not send reset link')
      logFrontendEvent({ category: 'password-reset', message: 'host_password_reset_requested', data: { deliveryMethod } })
      setMessage(deliveryMethod === 'sms' ? 'If that golf-course account exists and has a phone number, a reset link has been sent by SMS.' : 'If that golf-course account exists, a reset email has been sent.')
    } catch (err: any) {
        ...
        <form onSubmit={onSubmit} className="formStack" style={{ maxWidth: 560 }}>
          ...
          <fieldset className="formStack" style={{ gap: 8, border: 0, padding: 0, margin: 0 }}>
            ...
            <label className="small" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="radio" name="hostDeliveryMethod" value="email" checked={deliveryMethod === 'email'} onChange={() => setDeliveryMethod('email')} />
              Send by email
            </label>
            <label className="small" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="radio" name="hostDeliveryMethod" value="sms" checked={deliveryMethod === 'sms'} onChange={() => setDeliveryMethod('sms')} />
              Send by SMS
            </label>
          </fieldset>

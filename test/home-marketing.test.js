...
test('home presents only the mission and two responsive YouTube commercial sections', async () => {
  ...
  assert.match(home, /const \{ user, loading: authLoading \} = useAuth\(\)/)
  assert.match(home, /\{!authLoading && !user \? \(/)
  assert.match(home, /Browse videos/)
  assert.match(home, /marketing_video_library_browse_selected/)
  assert.match(home, /https:\/\/www\.facebook\.com\/people\/Golf-Homiez\/61593610114459\//)
  assert.match(home, /https:\/\/www\.instagram\.com\/golfhomiez\//)
  assert.match(home, /https:\/\/www\.youtube\.com\/@GolfHomiez/)
  assert.doesNotMatch(home, /linkedin|LinkedIn|linkedin\.com/i)
  assert.match(home, /className="homeMissionSocial"/)
  assert.match(home, /social_link_selected/)
  assert.match(home, /aria-label=\{`GolfHomiez on \$\{social\.label\}`\}/)
  ...
  assert.match(css, /\.homeMissionAside\{[\s\S]*align-items:flex-end[\s\S]*justify-content:space-between/)
  assert.match(css, /\.homeMissionSocial\{[\s\S]*justify-content:flex-end/)
  assert.match(css, /\.homeMissionSocialLink\{[\s\S]*border-radius:50%/)
  assert.match(css, /@media \(max-width:720px\)[\s\S]*\.homeMissionSocial\{[\s\S]*justify-content:flex-end/)

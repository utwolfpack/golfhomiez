...
test('requested admin and payment interfaces expose details and enforce one-at-a-time expansion', () => {
  ...
  assert.match(billing, /method\.lastFour \? `\$\{brand\} ending in \$\{method\.lastFour\}` : brand/)
  assert.match(billing, /Card details unavailable — update the card below/)
  assert.match(billing, /void refreshBillingStatus\(\)/)
  assert.match(billing, /Add or change payment method/)
  assert.match(billing, /\['active', 'trialing'\]\.includes/)
  assert.match(billing, /Reactivate subscription/)
...
test('Checkout return reconciles the exact session and Homie Tokens are admin-defined', () => {
  ...
  assert.match(billing, /method\?\.card\?\.brand \|\| method\?\.type \|\| null/)
  assert.match(billing, /return paymentMethod && !paymentMethod\.deleted \? paymentMethod : null/)
  assert.match(billing, /payment_method_types: \['card'\]/)
  assert.match(billing, /session\.mode === 'setup'/)
  assert.match(billing, /default_payment_method: paymentMethodId/)

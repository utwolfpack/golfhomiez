...
test('mobile team, notifications, host messaging, calendar, and Oquirrh Hills refinements are wired', () => {
  const teams = read('src/pages/Teams.tsx')
  const inbox = read('src/pages/Inbox.tsx')
  const messaging = read('src/components/TournamentMessagingPanel.tsx')
  const hostPortal = read('src/pages/HostPortal.tsx')
  const server = read('server/index.js')
  const calendar = read('src/pages/GolfCourseCalendarPage.tsx')
  const coursePage = read('src/pages/GolfCoursePage.tsx')
  const styles = read('src/index.css')
  const migration = read('migration_scripts/20260916_091_oquirrh_hills_hole_data_correction.sql')
  const migrations = read('server/migrations/index.js')

  assert.match(teams, /teamCreateMemberEmailField/)
  assert.match(teams, /TeamMemberCheckIcon/)
  assert.match(teams, /TeamMemberTrashIcon/)
  assert.match(styles, /\.teamMemberValidateIconButton\.validated/)
  assert.match(styles, /@media \(max-width:760px\)[\s\S]*\.teamCreateMemberEmailField/)

  assert.match(inbox, /notificationSendMessageButton/)
  assert.match(inbox, />Back to messages<\/button>/)
  assert.match(styles, /\.notificationSendMessageButton,\.notificationBackToMessagesButton/)

  assert.match(messaging, /window\.setInterval\(\(\) => \{ void loadHistory\(\{ quiet: true \}\) \}, 30000\)/)
  assert.match(server, /host_additional_account_invitation_email_sent/)
  assert.match(server, /invited you to represent/)
  assert.match(server, /Host name:/)
  assert.match(server, /Host email:/)
  assert.match(hostPortal, /invitationEmailSent/)

  assert.match(calendar, /golfCourseCalendarMobileModalOverlay/)
  assert.match(calendar, /role="dialog" aria-modal="true"/)
  assert.match(coursePage, /handleUpcomingItemSelected/)
  assert.match(coursePage, /window\.matchMedia\('\(max-width:760px\)'\)\.matches/)
  assert.match(coursePage, /golfCourseCalendarMobileModalOverlay/)

  assert.match(migration, /Oquirrh Hills Golf Course/)
  assert.match(migration, /tooele-city-correction/)
  assert.match(migration, /18, 72, 6591/)
  assert.match(migration, /oquirrh-tooele-default-18/)
  assert.match(migration, /oquirrh-tooele-copper-01/)
  assert.match(migration, /oquirrh-tooele-lead-01/)

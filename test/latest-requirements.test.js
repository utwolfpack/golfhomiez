...
test('nine-hole score completion and registration legal content are course-aware and branded', async () => {
  ...
  assert.match(legal, /createPortal\(/)
  assert.match(legal, /document\.body\.style\.overflow = 'hidden'/)
  assert.match(legal, /ref=\{closeButtonRef\}/)
  assert.match(styles, /\.registrationLegalModal/)
  assert.match(styles, /\.registrationLegalBrand/)
  assert.match(styles, /height:100dvh/)
  assert.match(styles, /env\(safe-area-inset-top\)/)
  assert.match(styles, /-webkit-overflow-scrolling:touch/)
  assert.match(styles, /\.registrationLegalClose\{[^}]*min-height:44px/s)
})

test('host course calendar events support recurring cadence options end-to-end', () => {
  const hostPortal = read('src/pages/HostPortal.tsx')
  const accounts = read('src/lib/accounts.ts')
  const server = read('server/index.js')
  const courseEvents = read('server/lib/course-events.js')
  const publicPages = read('server/lib/golf-course-public-pages.js')
  const migration = read('migration_scripts/20260918_094_course_event_recurrence.sql')
  const migrations = read('server/migrations/index.js')
  const packageJson = read('package.json')

  assert.match(hostPortal, /<option value="daily">Daily<\/option>/)
  assert.match(hostPortal, /<option value="weekly">Weekly<\/option>/)
  assert.match(hostPortal, /<option value="monthly">Monthly<\/option>/)
  assert.match(hostPortal, /<option value="yearly">Yearly<\/option>/)
  assert.match(hostPortal, /Repeat through/)
  assert.match(hostPortal, /host_course_event_recurrence_changed/)
  assert.match(accounts, /recurrenceCadence\?: CourseEventRecurrenceCadence/)
  assert.match(accounts, /recurrenceEndDate\?: string \| null/)
  assert.match(courseEvents, /expandRecurringCourseEvents/)
  assert.match(courseEvents, /recurrence_cadence/)
  assert.match(server, /recurrenceCadence: event\?\.recurrenceCadence/)
  assert.match(publicPages, /expandRecurring: true/)
  assert.match(migration, /ADD COLUMN recurrence_cadence/)
  assert.match(migration, /ADD COLUMN recurrence_end_date/)
  assert.match(migrations, /version: '20260918_094'/)
  assert.match(packageJson, /"postinstall": "npm run cleanup:project-files && npm run db:migrate && npm run build"/)
})

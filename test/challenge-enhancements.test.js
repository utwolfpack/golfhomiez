...
test('challenge UI implements profile state defaults, optional individual location, validated members, active editing, and participant-only leaderboard', () => {
  ...
  assert.match(source, /: Boolean\(individualChallengeDateRangeValid && individualChallengeLocationValid && parsedIndividualParticipantEmails\.length <= 24/)
...
test('individual challenge golfers choose their own course when the creator leaves course optional and leaderboard shows it', () => {
  ...
  assert.match(source, /Choose round details/)
  ...
  assert.match(source, /latestAllowedIndividualChallengeRoundDate\(endDate\)/)
  assert.match(source, /max=\{latestAllowedRoundDate \|\| undefined\}/)
  assert.match(source, /disabled=\{!dateWindowOpen\}/)
  assert.match(source, /individual_challenge_round_details_save_blocked/)
  assert.match(source, /validationReason: 'future_round_date'/)
  assert.match(source, /Round date must be today or earlier in your local time zone\./)
  assert.match(source, /cannot be after today/)
...
test('Team Challenge leaderboard uses the hole comparison layout and only shows Push for Skins - Push', () => {
  ...
  assert.match(challengesPage, /const valueColumnLabel = isDollarGame \? 'Dollars' : 'Points'/)
  assert.match(challengesPage, /\{showPointsColumn \? <span>\{valueColumnLabel\}<\/span> : null\}/)
...
test('latest challenge requirements add team row styling, skins-push dollars, individual round dates, participant feedback, and My Scores records', () => {
  const challengesPage = read('src/pages/Challenges.tsx')
  ...
  assert.match(challengesPage, /inboxChallengeLineItem--team/)
  assert.match(css, /\.inboxChallengeLineItem--team\{[\s\S]*background:#f3f8ff/)

  assert.match(challengesPage, /formatDollarAmount/)
  assert.match(challengesPage, /Dollars per hole/)
  assert.match(challengesPage, /formatSkinsPushNetWinnerLabel/)
  assert.match(challengesPage, /netWon > 0 \? `\$\{formatDollarAmount\(netWon\)\} won` : ''/)
  assert.match(challengesPage, /running_team_dollar_lead/)
  assert.match(roundDetail, /const valueColumnLabel = isDollarGame \? 'Dollars' : 'Points'/)
  assert.match(directions, /dollars per hole/)
  assert.match(directions, /carryover dollars/)

  assert.match(challengesPage, /id="individualParticipantRoundDate"/)
  assert.match(challengesPage, /min=\{startDate \|\| undefined\}/)
  assert.match(challengesPage, /max=\{latestAllowedRoundDate \|\| undefined\}/)
  assert.match(challengesPage, /dateSelectionRequired: isIndividualChallengeDateRange\(message\)/)
  assert.match(client, /roundDate\?: string \| null/)
  assert.match(server, /resolveIndividualChallengeRoundDate/)
  assert.match(server, /Select the date you played this Individual Challenge round/)
  assert.match(server, /roundDate must be between|Round date must be between/)
  assert.match(mysqlStorage, /nextParticipant\.roundDate = course\?\.roundDate \|\| null/)
  assert.match(sqliteStorage, /nextParticipant\.roundDate = course\?\.roundDate \|\| null/)
  assert.match(jsonStorage, /nextParticipant\.roundDate = course\?\.roundDate \|\| null/)

  assert.match(challengesPage, /individual_challenge_create_blocked_too_few_participants/)
  assert.match(challengesPage, /An Individual Challenge must have at least two participants, including you/)
  assert.match(challengesPage, /challengeSubmitFeedback/)
  assert.match(server, /individual_challenge_too_few_golfers/)
  assert.match(server, /participants\.length < 2/)
  assert.match(server, /enrichIndividualChallengeScoreRecordsForUser/)
  assert.match(server, /source: 'individual_challenge'/)
  assert.match(server, /individualChallengeScoreCount/)

  assert.match(myScores, /function isIndividualChallengeScore/)
  assert.match(myScores, /'Individual Challenge' : 'Solo Round'/)
  assert.match(myScores, /individual_challenge_score_records_loaded/)

...
test('golf user notifications preserve messages and Team Challenges while adding unified notification behavior', () => {
  ...
  assert.match(inboxFeatureFiles, /individualParticipantEmails/)
  assert.match(inboxFeatureFiles, /parsedIndividualParticipantEmails\.length\}\/25 golfers entered/)
  ...
  assert.match(server, /Only the golfer who created the challenge can complete it\./)
...
test('team challenge skins score entry supports optional messages, partial saves, close saves, and hides standard points column', () => {
  ...
  assert.match(challengesPage, /isTeamChallenge \? Boolean\(proposerTeamId && \/\^\\d\+\$\/\.test\(challengedTeamIdentifier\.trim\(\)\)\) : Boolean\(challengeBody\.trim\(\)/)
...
test('profile actions, leaderboard freshness, and mobile line-item views match the requested user experience', () => {
  ...
  assert.match(challenges, /const currentMessage = await fetchCurrentChallengeForLeaderboard\(message, 'individual_challenge', 'open'\)/)

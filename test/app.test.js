...
test('hole-by-hole input uses the scoreinput reference layout throughout the app', () => {
  ...
  assert.match(component, /function adjustActiveScore\(delta: number, source: 'decrease_button' \| 'increase_button'\)/)
  assert.match(component, /category: 'scorecard\.hole\.stroke_adjust'/)
  assert.match(component, /adjustActiveScore\(-1, 'decrease_button'\)/)
  assert.match(component, /adjustActiveScore\(1, 'increase_button'\)/)
  ...
  assert.match(css, /grid-template-columns:48px 72px 48px/)
  assert.match(css, /background:linear-gradient\(180deg, #3f92ff 0%, #1670ea 48%, #0752bf 100%\)/)
  assert.match(css, /\.holeInputScoreValueBlock\{[\s\S]*width:72px/)
  assert.match(css, /\.scoreStepperValue\{[\s\S]*width:72px[\s\S]*min-height:52px/)
  assert.match(component, /hasDisplayablePositiveNumber/)
})
...
test('finished round hole details use score words and requested scoring backgrounds', () => {
  ...
  assert.match(modal, /teamComparisonColumns: displayMode === 'team'/)
  ...
  assert.doesNotMatch(modal, /Hole-by-hole comparison:/)
  assert.match(modal, /<strong>Course Par:<\/strong> \{roundCoursePar \?\? '—'\}/)
  assert.match(modal, /<strong>Score:<\/strong> \{teamLabel\} \{holeScoreTotal\} • \{opponentLabel\} \{opponentHoleScoreTotal\}/)
...
test('round detail modal keeps solo holes single-sided and uses line-item team comparison review', () => {
  ...
  assert.match(modal, /const roundTypeLabel = isIndividualChallengeRound \? 'Individual Challenge' : displayMode === 'solo' \? 'Solo round' : isTeamChallengeRound \? 'Team Challenge' : 'Team round'/)
...
test('logged round views show incomplete hole-by-hole indicators', () => {
  ...
  assert.match(modal, /<strong>Total Strokes:<\/strong> \{\(round as any\)\.roundScore\}/)
  assert.match(modal, /<strong>Score:<\/strong> \{soloRelativeScoreLabel\}/)
  assert.match(modal, /holes\.length \? renderHoleDetails\(holes\) : <div className="small">No hole-by-hole detail saved<\/div>/)
...
test('my golf scores round viewer uses round-played dates, relative score, and course par metadata', () => {
  const modal = fs.readFileSync(new URL('../src/components/RoundDetailModal.tsx', import.meta.url), 'utf8')
  const server = fs.readFileSync(new URL('../server/index.js', import.meta.url), 'utf8')

  assert.match(modal, /formatRelativeToPar/)
  assert.match(modal, /const soloRelativeScoreLabel = soloRelativeToPar == null \? '—' : formatRelativeToPar\(soloRelativeToPar\)/)
  assert.match(modal, /<strong>Total Strokes:<\/strong> \{\(round as any\)\.roundScore\}/)
  assert.match(modal, /<strong>Score:<\/strong> \{soloRelativeScoreLabel\}/)
  assert.match(modal, /<strong>Date:<\/strong> \{formatFriendlyDate\(\(round as any\)\.date\)\}/)
  assert.doesNotMatch(modal, /Logged at:/)
  assert.doesNotMatch(modal, /formatFriendlyDateTime/)
  assert.match(modal, /<strong>Course Par:<\/strong> \{roundCoursePar \?\? '—'\}/)
  assert.match(modal, /canShowTeamComparison && detailView === 'round'/)
  assert.match(modal, /<strong>Score:<\/strong> \{teamLabel\} \{holeScoreTotal\} • \{opponentLabel\} \{opponentHoleScoreTotal\}/)
  assert.doesNotMatch(modal, /detailTeamScore/)
  assert.doesNotMatch(modal, /Hole-by-hole comparison:/)
  assert.match(modal, /displayDateSource: 'round\.date'/)
  assert.match(modal, /summaryLabels: displayMode === 'solo'/)

  const soloTotalStrokesIndex = modal.indexOf('<div><strong>Total Strokes:</strong>')
  const soloScoreIndex = modal.indexOf('<div><strong>Score:</strong> {soloRelativeScoreLabel}</div>')
  const soloDateIndex = modal.indexOf('<div><strong>Date:</strong> {formatFriendlyDate((round as any).date)}</div>', soloScoreIndex)
  assert.ok(soloTotalStrokesIndex >= 0 && soloScoreIndex > soloTotalStrokesIndex && soloDateIndex > soloScoreIndex, 'solo score should appear below Total Strokes and before Date')

  const teamCourseParIndex = modal.indexOf('<div><strong>Course Par:</strong>')
  const teamScoreIndex = modal.indexOf('<div><strong>Score:</strong> {teamLabel} {holeScoreTotal} • {opponentLabel} {opponentHoleScoreTotal}</div>')
  const teamResultIndex = modal.indexOf('<div><strong>Result:</strong>', teamScoreIndex)
  assert.ok(teamCourseParIndex >= 0 && teamScoreIndex > teamCourseParIndex && teamResultIndex > teamScoreIndex, 'team Course Par and Score should appear before Result')

  assert.match(server, /function deriveTeamChallengeCoursePar/)
  assert.match(server, /async function attachTeamChallengeCoursePars/)
  assert.match(server, /findGolfCourseForState\(state, course\)/)
  assert.match(server, /coursePar: deriveTeamChallengeCoursePar\(message\.proposerTeamHoles, message\.challengedTeamHoles\)/)
  assert.match(server, /const scores = await attachTeamChallengeCoursePars\(builtScores, req\)/)
  assert.match(server, /team_challenge_score_course_par_enrichment_completed/)
})
...
test('navigation uses styled dropdown items and exposes golfer Teams and Notifications navigation', () => {
  ...
  assert.match(nav, /to="\/host\/golfhomiezsite"/)
  assert.match(nav, /Golf Homiez Site/)
  ...
  assert.match(nav, /to="\/teams"/)
  assert.match(nav, />Teams<\/NavLink>/)
...
test('mobile banner navigation provides golfer shortcuts and replaces the account email with a Golf Homiez user icon', () => {
  ...
  assert.match(nav, /to: '\/teams', label: 'Teams'/)
  ...
  assert.match(nav, /function TeamsIcon/)
  assert.match(nav, /NotificationBellIcon/)
  ...
  assert.match(nav, /mobile_teams_selected/)
...
test('golfadmin portal is organized into Golf, Tournaments, API Usage, Marketing, Support, and Admin pages with Golf as the default', () => {
  const adminPortal = fs.readFileSync(new URL('../src/pages/AdminPortal.tsx', import.meta.url), 'utf8')
  const adminClient = fs.readFileSync(new URL('../src/lib/admin.ts', import.meta.url), 'utf8')
  const adminLib = fs.readFileSync(new URL('../server/lib/admin-portal.js', import.meta.url), 'utf8')
  ...
  assert.match(adminPortal, /type AdminPortalPage = 'golf' \| 'tournaments' \| 'api' \| 'marketing' \| 'support' \| 'admin'/)
  ...
  assert.match(adminPortal, /label: 'Support'/)
  ...
  assert.match(adminPortal, /data-admin-page="support"/)
...
test('host and organizer profile pages can update portal account details with correlated logging', () => {
  ...
  assert.match(app, /path="\/host\/golfhomiezsite"/)
  assert.match(app, /HostGolfHomiezSite/)
  ...
  assert.match(accounts, /export function fetchHostGolfHomiezSite/)
  assert.match(accounts, /export function updateHostGolfHomiezSite/)
  ...
  assert.match(server, /app\.get\('\/api\/host\/golfhomiez-site'/)
  assert.match(server, /app\.put\('\/api\/host\/golfhomiez-site'/)
...
test('profile pages keep legacy account website fields hidden while the dedicated Golf Homiez Site manages the public course website link', () => {
  const hostProfile = fs.readFileSync(new URL('../src/pages/HostProfile.tsx', import.meta.url), 'utf8')
  ...
  assert.doesNotMatch(hostProfile, /Course Website URL/)
  assert.doesNotMatch(hostProfile, /publicPage\.websiteUrl/)
  assert.match(hostSite, /Course Website URL/)
  assert.match(hostSite, /publicPage\.websiteUrl/)
...
test('admin scheduled jobs page, manual run API, migration, and dedicated logs are wired', () => {
  ...
  assert.match(adminLib, /PexelsQuotaMetadata/)
  assert.match(adminLib, /commercialMetadata\?: ScheduledJobCommercialMetadata/)
  assert.match(page, /Commercial metadata/)
  assert.match(page, /Pexels:/)
  assert.match(page, /requests remaining/)
  assert.match(page, /Download latest MP4/)
  assert.match(page, /scheduled_job_latest_mp4_download_clicked/)
  assert.match(page, /Media provided by Pexels/)
  ...
  assert.match(server, /getLatestScheduledJobCommercialOutput\(getPool\(\), jobId\)/)
  assert.match(server, /insideCommercialRoot/)
  assert.match(server, /res\.download\(absolutePath, fileName/)
  ...
  assert.match(scheduledJobs, /getLatestScheduledJobCommercialOutput/)
  assert.match(scheduledJobs, /commercialMetadataForJob/)
  assert.match(scheduledJobs, /pexelsQuota/)
  assert.match(scheduledJobStore, /getLatestSuccessfulScheduledJobRuns/)
  assert.match(scheduledJobStore, /status = 'success'/)
...
test('support page persists support tickets for users and admins with email notification, unread state, replies, close state, and logging', () => {
  const app = fs.readFileSync(new URL('../src/App.tsx', import.meta.url), 'utf8')
  const nav = fs.readFileSync(new URL('../src/components/NavBar.tsx', import.meta.url), 'utf8')
  ...
  const adminClient = fs.readFileSync(new URL('../src/lib/admin.ts', import.meta.url), 'utf8')
  const server = fs.readFileSync(new URL('../server/index.js', import.meta.url), 'utf8')
  const supportTickets = fs.readFileSync(new URL('../server/lib/support-tickets.js', import.meta.url), 'utf8')
  ...
  assert.match(supportPage, /Open support requests/)
  assert.match(supportPage, /supportTicketLineItem/)
  assert.match(supportPage, /Unread message from GolfHomiez admin/)
  assert.match(supportPage, /Add details or respond to GolfHomiez admin/)
  assert.match(supportPage, /Closed support history/)
  assert.match(supportPage, /closed and is now read-only/)
  assert.match(supportPage, /sendSupportMessage\(\{ subject: trimmedSubject, message: trimmedMessage \}\)/)
  assert.match(supportPage, /fetchSupportTickets\(\)/)
  assert.match(supportPage, /fetchSupportTicket\(ticket\.id\)/)
  assert.match(supportPage, /replyToSupportTicket\(selectedTicket\.id, trimmedMessage\)/)
  assert.match(supportPage, /support_ticket_create_started/)
  assert.match(supportPage, /support_ticket_reply_saved/)
  ...
  assert.match(supportClient, /\/api\/support\/tickets/)

  assert.match(adminPortal, /\{ id: 'support', label: 'Support' \}/)
  assert.match(adminPortal, /function SupportDashboardSection\(\)/)
  assert.match(adminPortal, /Respond to ticket/)
  assert.match(adminPortal, /Close ticket/)
  assert.match(adminPortal, /Closed support tickets/)
  assert.match(adminClient, /fetchAdminSupportTickets/)
  assert.match(adminClient, /replyToAdminSupportTicket/)
  assert.match(adminClient, /closeAdminSupportTicket/)

  assert.match(server, /SUPPORT_DESTINATION_EMAIL = 'golfhomiez@outlook\.com'/)
  assert.match(server, /app\.get\('\/api\/support\/tickets'/)
  assert.match(server, /app\.post\('\/api\/support\/messages'/)
  assert.match(server, /app\.post\('\/api\/support\/tickets\/:ticketId\/messages'/)
  assert.match(server, /app\.get\('\/api\/admin\/support\/tickets', adminMiddleware/)
  assert.match(server, /app\.post\('\/api\/admin\/support\/tickets\/:ticketId\/messages', adminMiddleware/)
  assert.match(server, /app\.post\('\/api\/admin\/support\/tickets\/:ticketId\/close', adminMiddleware/)
  ...
  assert.match(server, /admin_support_ticket_reply_saved/)
  assert.match(server, /admin_support_ticket_closed/)
  assert.match(supportTickets, /user_unread/)
  assert.match(supportTickets, /admin_unread/)
  assert.match(supportTickets, /Closed support tickets are read-only\./)
...
test('team challenge skins and skins-push points are persisted, logged, and shown on leaderboard', () => {
  ...
  assert.match(directionsPage, /each additional stroke adds the selected dollars-per-hole value again/)
...
test('host profile stays account-only while the dedicated Golf Homiez Site uses catalog defaults and uploaded banners', () => {
  const hostProfile = fs.readFileSync(new URL('../src/pages/HostProfile.tsx', import.meta.url), 'utf8')
  const hostSite = fs.readFileSync(new URL('../src/pages/HostGolfHomiezSite.tsx', import.meta.url), 'utf8')
  ...
  assert.doesNotMatch(hostProfile, /Course Website URL/)
  assert.doesNotMatch(hostProfile, /ImageUploadField/)

  assert.match(hostSite, /<label className="label">Course Website URL<\/label>/)
  assert.match(hostSite, /<label className="label">Public Contact Phone<\/label>/)
  assert.match(hostSite, /<label className="label">Street Address<\/label>/)
  assert.match(hostSite, /<label className="label">State<\/label>/)
  assert.match(hostSite, /<label className="label">Zip Code<\/label>/)
  assert.match(hostSite, /ImageUploadField/)
  assert.match(hostSite, /bannerImageData/)
  assert.match(hostSite, /defaultGolfCourseBanner/)
  assert.match(hostSite, /bannerImageData, bannerImageUrl: null/)
  assert.match(publicPage, /page\.bannerImageData \|\| defaultGolfCourseBanner/)
  assert.match(accounts, /bannerImageData\?: string \| null/)
  assert.match(accounts, /fetchHostGolfHomiezSite/)
  assert.match(accounts, /updateHostGolfHomiezSite/)

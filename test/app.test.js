...
test('mobile diagnostics patch adds preboot logging, diag pixel, and frontend log ingestion', () => {
  ...
  assert.match(indexHtml, /html_preboot_start/)
  assert.match(indexHtml, /asset_load_failure_preboot/)
  assert.match(indexHtml, /window_error_preboot/)
  assert.match(server, /app\.post\('\/api\/client-logs'/)
  assert.match(server, /status\(204\)\.end\(\)/)
  assert.match(server, /app\.get\('\/diag\/pixel\.gif'/)
  assert.match(logger, /frontend\.log/)
  assert.match(logger, /X-Correlation-Id/)
  assert.match(frontendLogger, /main_tsx_loaded/)
  assert.match(frontendLogger, /asset_load_failure_runtime/)
  assert.match(app, /app_component_mounted/)
  assert.match(app, /route_changed/)
  assert.match(main, /react_bootstrap_start/)
})
